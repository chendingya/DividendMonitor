 ...
 */
export async function migrateCorporateActionsCursorReset(db: SqliteDatabase): Promise<void> {
  const meta = (await db.prepare("SELECT value FROM app_settings WHERE key = 'migration:corporate_actions_cursor_reset_v1'").get()) as
    | { value: string }
    | undefined
  if (meta) return
